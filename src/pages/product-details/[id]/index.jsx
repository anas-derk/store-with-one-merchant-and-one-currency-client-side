...
export default function ProductDetails({ productIdAsProperty }) {
    ...
    const handleSelectRating = async (starNumber) => {
        try {
            if (Object.keys(userInfo).length === 0) {
                ...
                await axios.post(`${process.env.BASE_API_URL}/ratings/select-product-rating`, {
                                ...
                                <div className="row mb-3 justify-content-center align-items-center">
                                    ...
                                    <div className="col-lg-6">
                                        <div className="product-price-and-quantity me-3 mb-4 border-bottom border-2">
                                            ...
                                            <h5 className={`product-price mb-4 ${productInfo.discount !== 0 && "text-decoration-line-through"}`}>{productInfo.price.toFixed(2)} {t("EUR")}</h5>
                                            {
                                                productInfo.discount > 0 &&
                                                !isExistOfferOnProduct(currentDate, productInfo.startDiscountPeriod, productInfo.endDiscountPeriod) &&
                                                <h6 className="product-price-after-discount mb-3">{(productInfo.price - productInfo.discount).toFixed(2)} {t("EUR")}</h6>
                                            }
                                            {
                                                productInfo.discountInOfferPeriod > 0 &&
                                                isExistOfferOnProduct(currentDate, productInfo.startDiscountPeriod, productInfo.endDiscountPeriod) &&
                                                <h6 className="product-price-after-discount mb-4">{(productInfo.price - productInfo.discountInOfferPeriod).toFixed(2)} {t("EUR")}</h6>
                                            }
                                            {
