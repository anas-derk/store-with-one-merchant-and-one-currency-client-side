...
export default function Cart() {
    ...
    const [isGetProductsByIds, setIsGetProductsByIds] = useState(true);
    ...
    useEffect(() => {
        if (!isGetProductsByIds && !isGetUserInfo) {
            setIsLoadingPage(false);
        }
    }, [isGetProductsByIds, isGetUserInfo]);
        ...
        try {
            switch (operation) {
                case "increase-product-quantity": {
                    ...
                    setPricesDetailsSummary(calcTotalPrices(currentDate, result.data.products));
                    setAllProductsData(result.data.products);
                    break;
                }
        ...
        try {
            ...
            if (newProductsData.length > 0) {
                ...
                dispatch({
                    ...
                    productsCountInCart: result.data.products.length
                });
            } else {
            ...
            {!isLoadingPage && !errorMsgOnLoadingThePage && <>
                ...
                <div className="page-content pt-5">
                    <div className="container-fluid text-white text-center mb-4">
                        {allProductsData.length > 0 ? <section className="products-by-ids">
                                                        ...
                                                        <td className="product-cell">
                                                            <div className="row">
                                                                ...
                                                                <div className="col-lg-8">
                                                                    ...
                                                                    <h6 className={`product-price ${product.discount !== 0 || product.discountInOfferPeriod !== 0 ? "text-decoration-line-through" : ""}`}>{product.price.toFixed(2)} {t("EUR")}</h6>
                                                                    {
                                                                        product.discount > 0 &&
                                                                        !isExistOfferOnProduct(currentDate, product.startDiscountPeriod, product.endDiscountPeriod) &&
                                                                        <h6 className="product-price-before-discount m-0">{(product.price - product.discount).toFixed(2)} {t("EUR")}</h6>
                                                                    }
                                                                    {
                                                                        product.discountInOfferPeriod > 0 &&
                                                                        isExistOfferOnProduct(currentDate, product.startDiscountPeriod, product.endDiscountPeriod) &&
                                                                        <h6 className="product-price-after-discount m-0">{(product.price - product.discountInOfferPeriod).toFixed(2)} {t("EUR")}</h6>
                                                                    }
                                                                </div>
                                                            </div>
                                                        </td>
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            ...
                                                            <td className="product-cell">
                                                                ...
                                                                <h6 className="product-name mb-3">{product.name}</h6>
                                                                <h6 className={`product-price ${(product.discount !== 0 || product.discountInOfferPeriod) ? "text-decoration-line-through" : ""}`}>{product.price.toFixed(2)} {t("EUR")}</h6>
                                                                {
                                                                    product.discount > 0 &&
                                                                    !isExistOfferOnProduct(currentDate, product.startDiscountPeriod, product.endDiscountPeriod) &&
                                                                    <h6 className="product-price-after-discount m-0">{(product.price - product.discount).toFixed(2)} {t("EUR")}</h6>
                                                                }
                                                                {
                                                                    product.discountInOfferPeriod > 0 &&
                                                                    isExistOfferOnProduct(currentDate, product.startDiscountPeriod, product.endDiscountPeriod) &&
